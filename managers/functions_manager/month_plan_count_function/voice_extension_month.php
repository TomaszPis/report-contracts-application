...
function voice_extension_month($sfid, $start_mon, $end_mon){
	...
	try{
		$sql =	"SELECT COUNT(a.id_con) FROM contracts a
				 ...
				 WHERE f.or_source = FALSE AND e.id_off = 1 AND a.date_con >= '{$start_mon}' AND a.date_con <= '{$end_mon}' AND b.id_sfid = {$sfid};";
