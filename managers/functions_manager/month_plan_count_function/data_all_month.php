...
function data_all_month($sfid, $start_mon, $end_mon){
	...
	try{
		$sql =	"SELECT COUNT(a.id_con) FROM contracts a
				 ...
				 WHERE f.or_source = TRUE AND e.id_off = 2 AND a.date_con >= '{$start_mon}' AND a.date_con <= '{$end_mon}' AND b.id_sfid = {$sfid};";
