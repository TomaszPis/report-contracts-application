						...
						<table class="sfid-table">
							...
							<?php
								...
								foreach ($sfid as $s) {
									...
									$count_whole_month = voice_whole_month($s['id_sfid'], $start_mon, $end_mon);


									//Ile dziś podpisano pozyskań
									$today = voice_today($s['id_sfid'], $curr_day);

									//Ile podpisano pozyskań dzien wcześniej
									$yesterday = voice_yesterday($s['id_sfid'], $day_before);


									//Plany na pozyskanie voice dla sifd, na bierzący miesiąc
									$plan = voice_month_plan($s['id_sfid'], $m, $curr_year);
								...
								?>
								<tr>
									<td class="plan-name"><a href="?contracts_for_sfid&id_sfid=<?php echo $s['id_sfid']; ?>" class="sfid-link"><?php echo $s['sfid']; ?></a></td>
