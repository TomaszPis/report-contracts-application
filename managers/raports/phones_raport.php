						...
						<table class="sfid-table">
							...
							<?php
								...
								foreach ($sfid as $s) {
									...
									$count_whole_month = phones_all_month($s['id_sfid'], $start_mon, $end_mon);


									//Ile dziś podpisano pozyskań
									$today = phones_today($s['id_sfid'], $curr_day);

									//Ile podpisano pozyskań dzien wcześniej
									$yesterday = phones_yesterday($s['id_sfid'], $day_before);


									//Plany na pozyskanie phones dla sifd, na bierzący miesiąc
									$plan = phones_month_plan($s['id_sfid'], $m, $curr_year);
