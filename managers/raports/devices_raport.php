						...
						<table class="sfid-table">
							...
							<?php
								...
								foreach ($sfid as $s) {
									...
									$count_whole_month = devices_all_month($s['id_sfid'], $start_mon, $end_mon);


									//Ile dziś podpisano pozyskań
									$today = devices_today($s['id_sfid'], $curr_day);

									//Ile podpisano pozyskań dzien wcześniej
									$yesterday = devices_yesterday($s['id_sfid'], $day_before);


									//Plany na pozyskanie devices dla sifd, na bierzący miesiąc
									$plan = devices_month_plan($s['id_sfid'], $m, $curr_year);
