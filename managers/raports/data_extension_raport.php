						...
						<table class="sfid-table">
							...
							<?php
								...
								foreach ($sfid as $s) {
									...
									$count_whole_month = data_extension_month($s['id_sfid'], $start_mon, $end_mon);


									//Ile dziś podpisano pozyskań
									$today = data_extension_today($s['id_sfid'], $curr_day);

									//Ile podpisano pozyskań dzien wcześniej
									$yesterday = data_extension_yesterday($s['id_sfid'], $day_before);


									//Plany na pozyskanie data dla sifd, na bierzący miesiąc
									$plan = data_extension_plan($s['id_sfid'], $m, $curr_year);
