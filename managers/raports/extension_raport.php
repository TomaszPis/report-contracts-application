						...
						<table class="sfid-table">
							...
							<?php
								...
								foreach ($sfid as $s) {
									...
									$count_whole_month = voice_extension_month($s['id_sfid'], $start_mon, $end_mon);


									//Ile dziś podpisano pozyskań
									$today = voice_extension_today($s['id_sfid'], $curr_day);

									//Ile podpisano pozyskań dzien wcześniej
									$yesterday = voice_extension_yesterday($s['id_sfid'], $day_before);


									//Plany na pozyskanie voice dla sifd, na bierzący miesiąc
									$plan = voice_extension_plan($s['id_sfid'], $m, $curr_year);
