						...
						<table class="sfid-table">
							...
							<?php
								...
								foreach ($sfid as $s) {
									...
									$count_whole_month = data_all_month($s['id_sfid'], $start_mon, $end_mon);


									//Ile dziś podpisano pozyskań
									$today = data_today($s['id_sfid'], $curr_day);

									//Ile podpisano pozyskań dzien wcześniej
									$yesterday = data_yesterday($s['id_sfid'], $day_before);


									//Plany na pozyskanie data dla sifd, na bierzący miesiąc
									$plan = data_month_plan($s['id_sfid'], $m, $curr_year);
